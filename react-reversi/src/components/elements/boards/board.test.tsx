...
describe("Boardコンポーネント", () => {
  ...
  it("セルをクリックしたときにonCellClickが呼ばれること", () => {
    ...
    render(<Board boardState={createEmptyBoardState()} onCellClick={handleCellClick} />);
  ...
  it("配置可能な場所が正しく表示されること", () => {
    ...
    const placeable1 = screen.getByTestId("cell-1-2").querySelector('[data-testid="disc-none-can-place"]');
    const placeable2 = screen.getByTestId("cell-3-4").querySelector('[data-testid="disc-none-can-place"]');
  ...
  it("8x8以外のボードサイズでエラーメッセージが表示されること", () => {
    // 7x8の不正なボード
    const invalidBoard = Array(7).fill(null).map(() => 
