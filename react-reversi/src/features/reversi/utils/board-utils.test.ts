...
describe('board-utils関数', () => {
  ...
  describe('findFlippableDiscsInDirection関数', () => {
    it('ひっくり返せる石がある場合はその位置の配列を返すこと', () => {
      // 盤面を設定（DiscColor.Type.NONE: 空, DiscColor.Type.BLACK: 黒, DiscColor.Type.WHITE: 白）
    ...
    it('相手の石がない場合は空の配列を返すこと', () => {
      ...
      // 上方向をテスト (3,3)に黒(DiscColor.Type.BLACK)を置こうとしても置けない
      const direction: Direction = { rowDelta: -1, colDelta: 0 };
      const result = findFlippableDiscsInDirection(
        3,
        3,
    ...
    it('盤面の端を超える方向では空の配列を返すこと', () => {
      ...
      // 左上方向をテスト (0,0)に白(DiscColor.Type.WHITE)を置こうとしても盤面外になる
  ...
  describe('findFlippableDiscs関数', () => {
    it('すべての方向のひっくり返せる石を取得できること', () => {
      ...
      // (5,3)に黒(DiscColor.Type.BLACK)を置くと、白をひっくり返せる
      const result = findFlippableDiscs(5, 3, DiscColor.Type.BLACK, board);
    ...
    it('既に石がある場所には置けないこと', () => {
      ...
      // 既に石がある(3,3)に置こうとする
      const result = findFlippableDiscs(3, 3, DiscColor.Type.WHITE, board);
    ...
    it('ひっくり返せる石がない場合は空の配列を返すこと', () => {
      ...
      // 相手の石を挟めない場所
      const result = findFlippableDiscs(0, 0, DiscColor.Type.BLACK, board);
  ...
  describe('getPlaceablePositions関数', () => {
    it('初期盤面で黒が置ける位置を正しく取得できること', () => {
      // 初期状態の盤面を設定（4x4の例）
      const board = createBoardFromDiscColors([
        [DiscColor.Type.NONE, DiscColor.Type.NONE, DiscColor.Type.NONE, DiscColor.Type.NONE],
        [DiscColor.Type.NONE, DiscColor.Type.BLACK, DiscColor.Type.WHITE, DiscColor.Type.NONE],
        [DiscColor.Type.NONE, DiscColor.Type.WHITE, DiscColor.Type.BLACK, DiscColor.Type.NONE],
        [DiscColor.Type.NONE, DiscColor.Type.NONE, DiscColor.Type.NONE, DiscColor.Type.NONE],
      ]);

      // 黒(DiscColor.Type.BLACK)が置ける位置を取得
      const result = getPlaceablePositions(board, DiscColor.Type.BLACK);
    ...
    it('置ける場所がない場合は空の配列を返すこと', () => {
      // 黒が置ける場所がない盤面
      const board = createBoardFromDiscColors([
        [DiscColor.Type.BLACK, DiscColor.Type.BLACK, DiscColor.Type.BLACK],
        [DiscColor.Type.BLACK, DiscColor.Type.WHITE, DiscColor.Type.BLACK],
        [DiscColor.Type.BLACK, DiscColor.Type.BLACK, DiscColor.Type.BLACK],
      ]);

      // 白(DiscColor.Type.WHITE)が置ける位置を取得
      const result = getPlaceablePositions(board, DiscColor.Type.WHITE);
