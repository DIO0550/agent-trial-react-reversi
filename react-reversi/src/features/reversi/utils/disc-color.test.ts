...
describe('DiscColor', () => {
  describe('fromPlayerColor', () => {
    test('黒を選択したらDiscColor.Type.BLACKが返される', () => {
      const playerColor: PlayerColor = 'black';
      const result = DiscColor.fromPlayerColor(playerColor);
      expect(result).toBe(DiscColor.Type.BLACK);
    });

    test('白を選択したらDiscColor.Type.WHITEが返される', () => {
      const playerColor: PlayerColor = 'white';
      const result = DiscColor.fromPlayerColor(playerColor);
      expect(result).toBe(DiscColor.Type.WHITE);
    });
