...
describe("StartMenuコンポーネント", () => {
  ...
  it("デフォルトでは「普通」と「先行」が選択されていること", () => {
    ...
    expect(screen.getByText("普通").closest("button")).toHaveClass("border-blue-500");
    expect(screen.getByText("先行").closest("button")).toHaveClass("border-blue-500");
  });
  ...
  it("CPUレベルを選択できること", async () => {
    ...
    expect(screen.getByText("強い").closest("button")).toHaveClass("border-blue-500");
    expect(screen.getByText("普通").closest("button")).not.toHaveClass("border-blue-500");
  });
  ...
  it("手番を選択できること", async () => {
    ...
    expect(screen.getByText("後攻").closest("button")).toHaveClass("border-blue-500");
    expect(screen.getByText("先行").closest("button")).not.toHaveClass("border-blue-500");
  });
